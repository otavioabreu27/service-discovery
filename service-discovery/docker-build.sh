...
echo 
echo "/######################################################################/"
echo " Build new image otavioabreu27/service-discovery:${TAG_VERSION} "
echo "/######################################################################/"
echo

docker build $NO_CACHE -t "otavioabreu27/service-discovery:${TAG_VERSION}" -f Dockerfile .
...
if [[ ! "$SEND_TO_HUB" = "yes" ]]; then
    ...
    docker push "otavioabreu27/service-discovery:${TAG_VERSION}"
fi